"""Collector for Azure Database for MySQL Servers."""

import logging

from azure_client import AzureClient
from constants import (
    API_VERSIONS, OBJ_MYSQL, OBJ_RESOURCE_GROUP,
...
def collect_mysql_servers(client: AzureClient, result,
                          adapter_kind: str, subscriptions: list):
    """Collect MySQL servers across all subscriptions.

    Tries flexible servers first, falls back to single servers.
    """
    logger.info("Collecting MySQL servers")
    total = 0
    ...
    for sub in subscriptions:
        ...
        servers = client.get_all(
            path=(f"/subscriptions/{sub_id}/providers"
                  f"/Microsoft.DBforMySQL/flexibleServers"),
            api_version=API_VERSIONS["mysql_servers"],
        )
        ...
        if not servers:
            servers = client.get_all(
                path=(f"/subscriptions/{sub_id}/providers"
                      f"/Microsoft.DBforMySQL/servers"),
        ...
        for server in servers:
            ...
            safe_property(obj, SD_SERVICE, AZURE_SERVICE_NAMES.get(OBJ_MYSQL, ""))
            ...
            # MySQL-specific
            safe_property(obj, "innodb_buffer_pool_size",
                          str(props.get("innodbBufferPoolSize", "")))
    ...
    logger.info("Collected %d MySQL servers", total)

    if srv_objects:
        collect_metrics_for_objects(client, srv_objects, "mysql_servers")
